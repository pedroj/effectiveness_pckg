...
#' data(cecropia)
#' effectiveness_plot(
#'   q1 = cecropia$totvis, q2 = cecropia$totbic,
...
effectiveness_plot <- function(q1, q2,
  ...
  if (show.lines) {
    ...
    ## Calculate values ##
    df <- expand.grid(
      x = seq(x.lower - 0.05 * x.lower, x.upper + 0.05 * x.upper, length.out = 500),
    ...
    if (is.character(lines.breaks)) {
      lbreaks <- classInt::classIntervals(df$z,
        n = nlines + 1,
        style = lines.breaks
      )$brks
    }
  ...
  if (any(!is.na(label))) {
    effplot <- effplot +
      ggrepel::geom_text_repel(aes(x = .data$x, y = .data$y),
        data = d, size = label.size, label = label,
        # nudge_y = 0.5,
